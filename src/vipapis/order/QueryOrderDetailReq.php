...
class QueryOrderDetailReq {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['time_category'])){
				
				$this->time_category = $vals['time_category'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("time_category" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->time_category); 
	...
	public function write($output){
		...
		if($this->time_category !== null) {
			...
			$xfer += $output->writeFieldBegin('time_category');
			$xfer += $output->writeI32($this->time_category);
