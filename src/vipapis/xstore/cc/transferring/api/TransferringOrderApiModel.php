...
class TransferringOrderApiModel {
	...
	public $src_group_code = null;
	public $dest_group_code = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['src_group_code'])){
				
				$this->src_group_code = $vals['src_group_code'];
			}
			
			
			if (isset($vals['dest_group_code'])){
				
				$this->dest_group_code = $vals['dest_group_code'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("src_group_code" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->src_group_code);
			...
			if ("dest_group_code" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->dest_group_code);
	...
	public function write($output){
		...
		if($this->src_group_code !== null) {
			
			$xfer += $output->writeFieldBegin('src_group_code');
			$xfer += $output->writeString($this->src_group_code);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->dest_group_code !== null) {
			
			$xfer += $output->writeFieldBegin('dest_group_code');
			$xfer += $output->writeString($this->dest_group_code);
