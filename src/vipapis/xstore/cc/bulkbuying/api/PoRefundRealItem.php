...
class PoRefundRealItem {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['unique_codes'])){
				
				$this->unique_codes = $vals['unique_codes'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("unique_codes" == $schemeField){
				
				$needSkip = false;
				
				$this->unique_codes = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$this->unique_codes[$_size0++] = $elem0;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->unique_codes !== null) {
			
			$xfer += $output->writeFieldBegin('unique_codes');
			
			if (!is_array($this->unique_codes)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->unique_codes as $iter0){
