...
class SovOrder {
	...
	public $order_biz_flag_list = null;
	public $hebao_identification_code = null;
	public $is_create_transport = null;
	public $is_ship_or_cancel = null;
	public $is_print = null;
	public $order_type = null;
	public $street = null;
	public $pay_time = null;
	public $appoint_delivery_time = null;
	...
	public function __construct($vals=null){
		...
		if (!isset(self::$_TSPEC)){
			...
			37 => array(
			'var' => 'order_biz_flag_list'
			),
			38 => array(
			'var' => 'hebao_identification_code'
			),
			39 => array(
			'var' => 'is_create_transport'
			),
			40 => array(
			'var' => 'is_ship_or_cancel'
			),
			41 => array(
			'var' => 'is_print'
			),
		...
		if (is_array($vals)){
			...
			if (isset($vals['order_biz_flag_list'])){
				
				$this->order_biz_flag_list = $vals['order_biz_flag_list'];
			}
			
			
			if (isset($vals['hebao_identification_code'])){
				
				$this->hebao_identification_code = $vals['hebao_identification_code'];
			}
			
			
			if (isset($vals['is_create_transport'])){
				
				$this->is_create_transport = $vals['is_create_transport'];
			}
			
			
			if (isset($vals['is_ship_or_cancel'])){
				
				$this->is_ship_or_cancel = $vals['is_ship_or_cancel'];
			}
			...
			if (isset($vals['appoint_delivery_time'])){
				
				$this->appoint_delivery_time = $vals['appoint_delivery_time'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("order_biz_flag_list" == $schemeField){
				
				$needSkip = false;
				
				$this->order_biz_flag_list = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$this->order_biz_flag_list[$_size0++] = $elem0;
					}
					catch(\Exception $e){
			...
			if ("hebao_identification_code" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->hebao_identification_code);
			...
			if ("is_create_transport" == $schemeField){
				
				$needSkip = false;
				$input->readByte($this->is_create_transport); 
			...
			if ("is_ship_or_cancel" == $schemeField){
				
				$needSkip = false;
				$input->readByte($this->is_ship_or_cancel); 
			...
			if ("appoint_delivery_time" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->appoint_delivery_time);
	...
	public function write($output){
		...
		if($this->order_biz_flag_list !== null) {
			
			$xfer += $output->writeFieldBegin('order_biz_flag_list');
			
			if (!is_array($this->order_biz_flag_list)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->order_biz_flag_list as $iter0){
		...
		if($this->hebao_identification_code !== null) {
			
			$xfer += $output->writeFieldBegin('hebao_identification_code');
			$xfer += $output->writeString($this->hebao_identification_code);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->is_create_transport !== null) {
			
			$xfer += $output->writeFieldBegin('is_create_transport');
			$xfer += $output->writeByte($this->is_create_transport);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->is_ship_or_cancel !== null) {
			
			$xfer += $output->writeFieldBegin('is_ship_or_cancel');
			$xfer += $output->writeByte($this->is_ship_or_cancel);
		...
		if($this->appoint_delivery_time !== null) {
			
			$xfer += $output->writeFieldBegin('appoint_delivery_time');
			$xfer += $output->writeString($this->appoint_delivery_time);
