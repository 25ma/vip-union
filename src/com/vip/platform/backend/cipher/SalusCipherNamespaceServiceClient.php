...
class SalusCipherNamespaceServiceClient{
	...
	public static function getService(){
		...
		if (! SalusCipherNamespaceServiceClient::$isRegis){
			
			SalusCipherNamespaceServiceClient::regis ();
			SalusCipherNamespaceServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = SalusCipherNamespaceServiceClient::$DEFAULT_PROXY_IP;
		$port = SalusCipherNamespaceServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\platform\backend\cipher\SalusCipherNamespaceServiceHelper();
	}
