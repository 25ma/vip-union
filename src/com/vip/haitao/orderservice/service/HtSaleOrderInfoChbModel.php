...
class HtSaleOrderInfoChbModel {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['groupSizeSn'])){
				
				$this->groupSizeSn = $vals['groupSizeSn'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("groupSizeSn" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->groupSizeSn);
	...
	public function write($output){
		...
		if($this->groupSizeSn !== null) {
			
			$xfer += $output->writeFieldBegin('groupSizeSn');
			$xfer += $output->writeString($this->groupSizeSn);
		...
		if($this->gnum !== null) {
			...
			$xfer += $output->writeFieldBegin('gnum');
			$xfer += $output->writeI32($this->gnum);
