...
class WpcChannelBrandVO {
	...
	public function read($input){
		...
		while(true){
			...
			if ("goods" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem1 = new \com\vip\wpc\ospservice\channel\vo\WpcChannelBrandGoodVO();
						$elem1->read($input);
			...
			if ("storeQualificationImgList" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem2 = new \com\vip\wpc\ospservice\channel\vo\WpcStoreQualificationImgVO();
						$elem2->read($input);
						
						$this->storeQualificationImgList[$_size2++] = $elem2;
					}
					catch(\Exception $e){
			...
			if ("adCategoryList" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem3 = new \com\vip\wpc\ospservice\channel\vo\WpcChannelAdCategoryVO();
						$elem3->read($input);
						
						$this->adCategoryList[$_size3++] = $elem3;
					}
					catch(\Exception $e){
			...
			if ("couponList" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem5 = new \com\vip\wpc\ospservice\normality\vo\WpcCouponVo();
						$elem5->read($input);
						
						$this->couponList[$_size5++] = $elem5;
					}
					catch(\Exception $e){
			...
			if ("notice" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->notice);
	...
	public function write($output){
		...
		if($this->notice !== null) {
			...
			$xfer += $output->writeFieldBegin('notice');
