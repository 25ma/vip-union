...
class WpcChannelSizeStockVO {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['stockStatus'])){
				
				$this->stockStatus = $vals['stockStatus'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("stockStatus" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->stockStatus); 
	...
	public function write($output){
		...
		if($this->stockStatus !== null) {
			...
			$xfer += $output->writeFieldBegin('stockStatus');
			$xfer += $output->writeI32($this->stockStatus);
