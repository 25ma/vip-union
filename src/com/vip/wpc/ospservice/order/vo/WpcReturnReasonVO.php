...
class WpcReturnReasonVO {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['uploadImgFlag'])){
				
				$this->uploadImgFlag = $vals['uploadImgFlag'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("uploadImgFlag" == $schemeField){
				
				$needSkip = false;
				$input->readBool($this->uploadImgFlag);
	...
	public function write($output){
		...
		if($this->uploadImgFlag !== null) {
			
			$xfer += $output->writeFieldBegin('uploadImgFlag');
			$xfer += $output->writeBool($this->uploadImgFlag);
