...
class TaxOrderHeader {
	...
	public $invoicePayee = null;
	public $invoiceChecher = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['invoicePayee'])){
				
				$this->invoicePayee = $vals['invoicePayee'];
			}
			
			
			if (isset($vals['invoiceChecher'])){
				
				$this->invoiceChecher = $vals['invoiceChecher'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("invoicePayee" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->invoicePayee);
			...
			if ("invoiceChecher" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->invoiceChecher);
	...
	public function write($output){
		...
		if($this->invoicePayee !== null) {
			
			$xfer += $output->writeFieldBegin('invoicePayee');
			$xfer += $output->writeString($this->invoicePayee);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->invoiceChecher !== null) {
			
			$xfer += $output->writeFieldBegin('invoiceChecher');
			$xfer += $output->writeString($this->invoiceChecher);
