...
class DownloadElectronicInvoiceResModel {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['xmlContent'])){
				
				$this->xmlContent = $vals['xmlContent'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("xmlContent" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->xmlContent);
	...
	public function write($output){
		...
		if($this->xmlContent !== null) {
			...
			$xfer += $output->writeFieldBegin('xmlContent');
			$xfer += $output->writeString($this->xmlContent);
