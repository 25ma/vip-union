...
class GlVoucherModel {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['applyEmpNo'])){
				
				$this->applyEmpNo = $vals['applyEmpNo'];
			}
			
			
			if (isset($vals['currencyRate'])){
				
				$this->currencyRate = $vals['currencyRate'];
			}
			...
			if (isset($vals['creatorOa'])){
				
				$this->creatorOa = $vals['creatorOa'];
			}
			
			
			if (isset($vals['approverOa'])){
				
				$this->approverOa = $vals['approverOa'];
			}
			
			
			if (isset($vals['postOa'])){
				
				$this->postOa = $vals['postOa'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("applyEmpNo" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->applyEmpNo);
			...
			if ("currencyRate" == $schemeField){
				
				$needSkip = false;
				$input->readDouble($this->currencyRate);
			...
			if ("creatorOa" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->creatorOa);
			...
			if ("approverOa" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->approverOa);
			...
			if ("postOa" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->postOa);
	...
	public function write($output){
		...
		if($this->applyEmpNo !== null) {
			
			$xfer += $output->writeFieldBegin('applyEmpNo');
			$xfer += $output->writeString($this->applyEmpNo);
		...
		if($this->currencyRate !== null) {
			...
			$xfer += $output->writeFieldBegin('currencyRate');
			$xfer += $output->writeDouble($this->currencyRate);
		...
		if($this->creatorOa !== null) {
			
			$xfer += $output->writeFieldBegin('creatorOa');
			$xfer += $output->writeString($this->creatorOa);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->approverOa !== null) {
			
			$xfer += $output->writeFieldBegin('approverOa');
			$xfer += $output->writeString($this->approverOa);
		...
		if($this->postOa !== null) {
			...
			$xfer += $output->writeFieldBegin('postOa');
			$xfer += $output->writeString($this->postOa);
