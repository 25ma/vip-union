...
class CcPo {
	...
	public function read($input){
		...
		while(true){
			...
			if ("poExt" == $schemeField){
				
				$needSkip = false;
				
				$this->poExt = new \com\vip\xstore\cc\bulkbuying\CcPoExt();
				$this->poExt->read($input);
	...
	public function write($output){
		...
		if($this->poExt !== null) {
			...
			$xfer += $output->writeFieldBegin('poExt');
			
			if (!is_object($this->poExt)) {
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$xfer += $this->poExt->write($output);
