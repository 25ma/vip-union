...
class DiagramRequest {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['sqlVisible'])){
				
				$this->sqlVisible = $vals['sqlVisible'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("sortingModels" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem4 = new \com\vip\bip\report\service\SortingModel();
						$elem4->read($input);
						
						$this->sortingModels[$_size4++] = $elem4;
					}
					catch(\Exception $e){
			...
			if ("sqlVisible" == $schemeField){
				
				$needSkip = false;
				$input->readBool($this->sqlVisible);
	...
	public function write($output){
		...
		if($this->sqlVisible !== null) {
			
			$xfer += $output->writeFieldBegin('sqlVisible');
			$xfer += $output->writeBool($this->sqlVisible);
