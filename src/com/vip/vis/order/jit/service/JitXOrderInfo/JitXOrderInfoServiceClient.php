...
class JitXOrderInfoServiceClient{
	...
	public static function getService(){
		...
		if (! JitXOrderInfoServiceClient::$isRegis){
			
			JitXOrderInfoServiceClient::regis ();
			JitXOrderInfoServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = JitXOrderInfoServiceClient::$DEFAULT_PROXY_IP;
		$port = JitXOrderInfoServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\vis\order\jit\service\JitXOrderInfo\JitXOrderInfoServiceHelper();
	}
