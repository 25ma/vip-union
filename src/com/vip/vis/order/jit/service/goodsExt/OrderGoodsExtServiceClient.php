...
class OrderGoodsExtServiceClient{
	...
	public static function getService(){
		...
		if (! OrderGoodsExtServiceClient::$isRegis){
			
			OrderGoodsExtServiceClient::regis ();
			OrderGoodsExtServiceClient::$isRegis = true;
		}
		
		
		$ctx = \Osp\Context\InvocationContextFactory::getInstance ();
		$ip = OrderGoodsExtServiceClient::$DEFAULT_PROXY_IP;
		$port = OrderGoodsExtServiceClient::$DEFAULT_PROXY_PORT;	
		try{
		...
		return new \com\vip\vis\order\jit\service\goodsExt\OrderGoodsExtServiceHelper();
	}
