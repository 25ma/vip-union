...
interface ShipmentServiceIf{
	...
	public function getTpVendorWarehouseListRelations( $vendorCode, $warehouseCodeList);
...
class _ShipmentServiceClient extends \Osp\Base\OspStub implements \com\vip\vis\prp\shipment\service\ShipmentServiceIf{
	...
	public function getTpVendorWarehouseListRelations( $vendorCode, $warehouseCodeList){
		...
		$this->send_getTpVendorWarehouseListRelations( $vendorCode, $warehouseCodeList);
		return $this->recv_getTpVendorWarehouseListRelations();
	}
	
	public function send_getTpVendorWarehouseListRelations( $vendorCode, $warehouseCodeList){
		
		$this->initInvocation("getTpVendorWarehouseListRelations");
		$args = new \com\vip\vis\prp\shipment\service\ShipmentService_getTpVendorWarehouseListRelations_args();
		
		$args->vendorCode = $vendorCode;
		
		$args->warehouseCodeList = $warehouseCodeList;
		
		$this->send_base($args);
	}
	
	public function recv_getTpVendorWarehouseListRelations(){
		
		$result = new \com\vip\vis\prp\shipment\service\ShipmentService_getTpVendorWarehouseListRelations_result();
		$this->receive_base($result);
		if ($result->success !== null){
...
class ShipmentService_getTpVendorWarehouseListRelations_args {
		...
		if(true) {
			...
			while(true){
				...
				try{
					...
					$this->warehouseCodeList[$_size0++] = $elem0;
				}
				catch(\Exception $e){
...
class ShipmentService_getTpVendorWarehouseListRelations_result {
