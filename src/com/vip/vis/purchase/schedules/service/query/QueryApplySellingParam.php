...
class QueryApplySellingParam {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['vendorCodeList'])){
				
				$this->vendorCodeList = $vals['vendorCodeList'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("vendorCodeList" == $schemeField){
				
				$needSkip = false;
				
				$this->vendorCodeList = array();
				$_size4 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$this->vendorCodeList[$_size4++] = $elem4;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->vendorCodeList !== null) {
			
			$xfer += $output->writeFieldBegin('vendorCodeList');
			
			if (!is_array($this->vendorCodeList)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->vendorCodeList as $iter0){
