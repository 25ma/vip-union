...
class ProductSaleInfosParam {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['childOrderSns'])){
				
				$this->childOrderSns = $vals['childOrderSns'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("childOrderSns" == $schemeField){
				
				$needSkip = false;
				
				$this->childOrderSns = array();
				$_size7 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$this->childOrderSns[$_size7++] = $elem7;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->childOrderSns !== null) {
			
			$xfer += $output->writeFieldBegin('childOrderSns');
			
			if (!is_array($this->childOrderSns)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->childOrderSns as $iter0){
