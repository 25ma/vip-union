...
class ReceiveContainerItemsReq {
	...
	public $containerItemInfos = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['containerItemInfos'])){
				
				$this->containerItemInfos = $vals['containerItemInfos'];
			}
	...
	public function getName(){
		...
		return 'ReceiveContainerItemsReq';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("containerItemInfos" == $schemeField){
				
				$needSkip = false;
				
				$this->containerItemInfos = array();
				$_size1 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem1 = new \com\vip\vsc\oms\osp\ship\service\ContainerItemInfo();
						$elem1->read($input);
						
						$this->containerItemInfos[$_size1++] = $elem1;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->containerItemInfos !== null) {
			
			$xfer += $output->writeFieldBegin('containerItemInfos');
			
			if (!is_array($this->containerItemInfos)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->containerItemInfos as $iter0){
