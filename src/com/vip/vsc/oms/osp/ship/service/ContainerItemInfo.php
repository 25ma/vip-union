...
class ContainerItemInfo {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['shipQty'])){
				
				$this->shipQty = $vals['shipQty'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("shipQty" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->shipQty); 
	...
	public function write($output){
		...
		$xfer += $output->writeFieldBegin('shipQty');
		$xfer += $output->writeI32($this->shipQty);
