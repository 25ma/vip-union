...
class PoInfos {
	...
	public $shippingFromCode = null;
	public $shippingFromName = null;
	...
	public $airReasonType = null;
	public $airReasonRemark = null;
	...
	public function __construct($vals=null){
		...
		if (!isset(self::$_TSPEC)){
			...
			65 => array(
			'var' => 'shippingFromCode'
			),
			66 => array(
			'var' => 'shippingFromName'
			),
			67 => array(
			'var' => 'purchaseModel'
			),
		...
		if (is_array($vals)){
			...
			if (isset($vals['shippingFromCode'])){
				
				$this->shippingFromCode = $vals['shippingFromCode'];
			}
			
			
			if (isset($vals['shippingFromName'])){
				
				$this->shippingFromName = $vals['shippingFromName'];
			}
			...
			if (isset($vals['buyerEmail'])){
				
				$this->buyerEmail = $vals['buyerEmail'];
			}
			...
			if (isset($vals['airReasonType'])){
				
				$this->airReasonType = $vals['airReasonType'];
			}
			
			
			if (isset($vals['airReasonRemark'])){
				
				$this->airReasonRemark = $vals['airReasonRemark'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("shippingFromCode" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->shippingFromCode);
			...
			if ("shippingFromName" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->shippingFromName);
			...
			if ("bizTags" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$names = \com\vip\venus\visPo\service\PoBizTagsType::$__names;
						$name = null;
						$input->readString($name);
						foreach ($names as $k => $v){
			...
			if ("buyerEmail" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->buyerEmail);
			...
			if ("airReasonType" == $schemeField){
				
				$needSkip = false;
				$input->readByte($this->airReasonType); 
			...
			if ("airReasonRemark" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->airReasonRemark);
			...
			if ("transportType" == $schemeField){
				...
				$needSkip = false;
				$input->readByte($this->transportType); 
	...
	public function write($output){
		...
		if($this->shippingFromCode !== null) {
			
			$xfer += $output->writeFieldBegin('shippingFromCode');
			$xfer += $output->writeString($this->shippingFromCode);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->shippingFromName !== null) {
			
			$xfer += $output->writeFieldBegin('shippingFromName');
			$xfer += $output->writeString($this->shippingFromName);
		...
		if($this->bizTags !== null) {
			...
			foreach ($this->bizTags as $iter0){
				...
				$em = new \com\vip\venus\visPo\service\PoBizTagsType; 
		...
		if($this->buyerEmail !== null) {
			...
			$xfer += $output->writeFieldBegin('buyerEmail');
			$xfer += $output->writeString($this->buyerEmail);
		...
		if($this->isLuxury !== null) {
			...
			$xfer += $output->writeI32($this->isLuxury);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->airReasonType !== null) {
			
			$xfer += $output->writeFieldBegin('airReasonType');
			$xfer += $output->writeByte($this->airReasonType);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->airReasonRemark !== null) {
			
			$xfer += $output->writeFieldBegin('airReasonRemark');
			$xfer += $output->writeString($this->airReasonRemark);
		...
		if($this->transportType !== null) {
			...
			$xfer += $output->writeByte($this->transportType);
