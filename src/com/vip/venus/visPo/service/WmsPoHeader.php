...
class WmsPoHeader {
	...
	public $transferCenterWarehouse = null;
	public $wmsReceiveQtyTotal = null;
	public $firstReceiveTime = null;
	public $expectedArrivalDate = null;
	public $poQtyTotal = null;
	...
	public function __construct($vals=null){
		...
		if (!isset(self::$_TSPEC)){
			...
			55 => array(
			'var' => 'transferCenterWarehouse'
			),
			56 => array(
			'var' => 'wmsReceiveQtyTotal'
			),
			57 => array(
			'var' => 'firstReceiveTime'
			),
			58 => array(
		...
		if (is_array($vals)){
			...
			if (isset($vals['transferCenterWarehouse'])){
				
				$this->transferCenterWarehouse = $vals['transferCenterWarehouse'];
			}
			
			
			if (isset($vals['wmsReceiveQtyTotal'])){
				
				$this->wmsReceiveQtyTotal = $vals['wmsReceiveQtyTotal'];
			}
			
			
			if (isset($vals['firstReceiveTime'])){
				
				$this->firstReceiveTime = $vals['firstReceiveTime'];
			}
			...
			if (isset($vals['poQtyTotal'])){
				
				$this->poQtyTotal = $vals['poQtyTotal'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("transferCenterWarehouse" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->transferCenterWarehouse);
			...
			if ("wmsReceiveQtyTotal" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->wmsReceiveQtyTotal); 
			...
			if ("firstReceiveTime" == $schemeField){
				
				$needSkip = false;
				$input->readI64($this->firstReceiveTime);
			...
			if ("expectedArrivalDate" == $schemeField){
				...
				$needSkip = false;
				$input->readI64($this->expectedArrivalDate);
			...
			if ("poQtyTotal" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->poQtyTotal); 
	...
	public function write($output){
		...
		if($this->transferCenterWarehouse !== null) {
			
			$xfer += $output->writeFieldBegin('transferCenterWarehouse');
			$xfer += $output->writeString($this->transferCenterWarehouse);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->wmsReceiveQtyTotal !== null) {
			
			$xfer += $output->writeFieldBegin('wmsReceiveQtyTotal');
			$xfer += $output->writeI32($this->wmsReceiveQtyTotal);
		...
		if($this->firstReceiveTime !== null) {
			...
			$xfer += $output->writeFieldBegin('firstReceiveTime');
			$xfer += $output->writeI64($this->firstReceiveTime);
		...
		if($this->expectedArrivalDate !== null) {
			...
			$xfer += $output->writeI64($this->expectedArrivalDate);
			
			$xfer += $output->writeFieldEnd();
		}
		
		
		if($this->poQtyTotal !== null) {
			
			$xfer += $output->writeFieldBegin('poQtyTotal');
			$xfer += $output->writeI32($this->poQtyTotal);
