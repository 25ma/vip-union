...
class WmsPoLine {
	...
	public $exceptionDesc = null;
	public $itemDesc = null;
	public $tagPrice = null;
	public $uomCode = null;
	public $wmsReceiveQty = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['exceptionCode'])){
				
				$this->exceptionCode = $vals['exceptionCode'];
			}
			
			
			if (isset($vals['exceptionDesc'])){
				
				$this->exceptionDesc = $vals['exceptionDesc'];
			}
			...
			if (isset($vals['wmsReceiveQty'])){
				
				$this->wmsReceiveQty = $vals['wmsReceiveQty'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("exceptionCode" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->exceptionCode);
			...
			if ("exceptionDesc" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->exceptionDesc);
			...
			if ("wmsReceiveQty" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->wmsReceiveQty); 
	...
	public function write($output){
		...
		if($this->exceptionCode !== null) {
			...
			$xfer += $output->writeFieldBegin('exceptionCode');
			$xfer += $output->writeString($this->exceptionCode);
		...
		if($this->exceptionDesc !== null) {
			...
			$xfer += $output->writeFieldBegin('exceptionDesc');
			$xfer += $output->writeString($this->exceptionDesc);
		...
		if($this->wmsReceiveQty !== null) {
			
			$xfer += $output->writeFieldBegin('wmsReceiveQty');
			$xfer += $output->writeI32($this->wmsReceiveQty);
