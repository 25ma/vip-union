...
class ProductAndSkuChangeInfo {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['luxuryUpdateTime'])){
				
				$this->luxuryUpdateTime = $vals['luxuryUpdateTime'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("luxuryUpdateTime" == $schemeField){
				
				$needSkip = false;
				$input->readI64($this->luxuryUpdateTime); 
	...
	public function write($output){
		...
		if($this->luxuryUpdateTime !== null) {
			
			$xfer += $output->writeFieldBegin('luxuryUpdateTime');
			$xfer += $output->writeI64($this->luxuryUpdateTime);
