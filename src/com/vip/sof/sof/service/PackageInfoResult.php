...
class PackageInfoResult {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['abroadTrackList'])){
				
				$this->abroadTrackList = $vals['abroadTrackList'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("transportInfo" == $schemeField){
				...
				$this->transportInfo = new \com\vip\sof\sof\service\TransportInfoResult();
			...
			if ("goods" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem1 = new \com\vip\sof\sof\service\GoodsResult();
						$elem1->read($input);
			...
			if ("trackList" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem2 = new \com\vip\sof\sof\service\TrackItem();
						$elem2->read($input);
						
						$this->trackList[$_size2++] = $elem2;
					}
					catch(\Exception $e){
			...
			if ("abroadTrackList" == $schemeField){
				
				$needSkip = false;
				
				$this->abroadTrackList = array();
				$_size3 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem3 = new \com\vip\sof\sof\service\TrackItem();
						$elem3->read($input);
						
						$this->abroadTrackList[$_size3++] = $elem3;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->abroadTrackList !== null) {
			
			$xfer += $output->writeFieldBegin('abroadTrackList');
			
			if (!is_array($this->abroadTrackList)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->abroadTrackList as $iter0){
