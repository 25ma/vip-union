...
class CheckTrustInvoiceFromVEIParam {
	
	static $_TSPEC;
	public $appName = null;
	public $count = null;
	public $maxCount = null;
	public $timeLater = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['timeLater'])){
				
				$this->timeLater = $vals['timeLater'];
			}
	...
	public function getName(){
		...
		return 'CheckTrustInvoiceFromVEIParam';
	}
	...
	public function read($input){
		...
		while(true){
			...
			if ("timeLater" == $schemeField){
				
				$needSkip = false;
				$input->readI32($this->timeLater); 
	...
	public function write($output){
		...
		if($this->timeLater !== null) {
			
			$xfer += $output->writeFieldBegin('timeLater');
			$xfer += $output->writeI32($this->timeLater);
