...
class QueryDelayedResult {
	
	static $_TSPEC;
	public $orderSn = null;
	public $storeId = null;
	public $expectedShipTime = null;
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['expectedShipTime'])){
				
				$this->expectedShipTime = $vals['expectedShipTime'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("expectedShipTime" == $schemeField){
				
				$needSkip = false;
				$input->readString($this->expectedShipTime);
			...
			if ("transportList" == $schemeField){
				...
				while(true){
					...
					try{
						...
						$elem0 = new \com\vip\sof\sof\service\DelayedTransport();
						$elem0->read($input);
						
						$this->transportList[$_size0++] = $elem0;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->expectedShipTime !== null) {
			
			$xfer += $output->writeFieldBegin('expectedShipTime');
			$xfer += $output->writeString($this->expectedShipTime);
